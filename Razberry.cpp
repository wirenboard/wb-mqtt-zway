...
static std::string readInputTestFile(const char* path)
{
    ...
    buffer[size] = 0;
    if (fread(buffer, 1, size, file) == (unsigned long)size) text = buffer;
    fclose(file);
    delete[] buffer;
    return text;
}
...
static time_t mytime(time_t* _Time)
{
    ...
    if (acttime < m_lasttime) return m_lasttime;
    m_lasttime = acttime;
    return acttime;
}
...
bool CRazberry::GetUpdates()
{
    ...
    for (Json::Value::iterator itt = root.begin(); itt != root.end(); ++itt) {
        ...
        if (kName == "updateTime") {
            ...
            if (results.size() > 1) {
                if (kName.find("lastReceived") == std::string::npos) UpdateDevice(kName, obj);
            }
        }
    }
...
void CRazberry::InsertControl(_tZWaveDevice device, const string& ctrl_id)
{
    ...
    case ZDTYPE_SENSOR_POWER: {
        if (output_vector.size() < 1) output_vector.push_back(make_pair("type", "power"));
    }
    ...
    case ZDTYPE_SENSOR_TEMPERATURE: {
        if (output_vector.size() < 1) output_vector.push_back(make_pair("type", "temperature"));
    }
    ...
    case ZDTYPE_SENSOR_VOLTAGE: {
        if (output_vector.size() < 1) output_vector.push_back(make_pair("type", "voltage"));
    }
    case ZDTYPE_SENSOR_AMPERE: {
        if (output_vector.size() < 1) output_vector.push_back(make_pair("type", "generic_value"));
    }
    case ZDTYPE_SENSOR_PERCENTAGE: {
        if (output_vector.size() < 1) output_vector.push_back(make_pair("type", "generic_value"));
        control_id = to_string(device.instanceID) + "_" + to_string(device.commandClassID) + "_" +
                     to_string(device.scaleID);
        char buff[10];
        sprintf(buff, "%.2f", device.floatValue);
        if (value == "") value = buff;
        break;
...
void CRazberry::parseDevices(const Json::Value& devroot)
{
    for (Json::Value::iterator itt = devroot.begin(); itt != devroot.end(); ++itt) {
        ...
        if ((nodeID == 255) || (nodeID == m_controllerID)) continue; // skip ourself
        ...
        for (Json::Value::iterator ittInstance = nodeInstances.begin();
            ...
            if ((_device.instanceID == 0) && (haveMultipleInstance))
                continue; // We skip instance 0 if there are more, since it should be mapped to
                          // other instances or their superposition

            const Json::Value instance = (*ittInstance);

            if (_device.hasBattery) {
                _device.batValue =
                    instance["commandClasses"]["128"]["data"]["last"]["value"].asInt();
                string control_id = to_string(_device.instanceID) + "_128";
                ;
                vector<TPublishPair> output_vector;
                output_vector.push_back(
                    make_pair("type", "generic_value")); // FIXME type for battery ?
            ...
            if (instance["commandClasses"]["38"].empty() == false) {
                ...
                _device.intvalue =
                    instance["commandClasses"]["38"]["data"]["level"]["value"].asInt();
                ...
                _device.intvalue =
                    instance["commandClasses"]["37"]["data"]["level"]["value"].asInt();
                InsertControl(_device, "");
            }
            ...
            if (instance["commandClasses"]["48"].empty() == false) {
                ...
                if (instance["commandClasses"]["48"]["data"]["level"].empty() == false) {
                    _device.intvalue =
                        instance["commandClasses"]["48"]["data"]["level"]["value"].asInt();
                    string control_id =
                        to_string(_device.instanceID) + "_" + to_string(_device.commandClassID);
                    ...
                    for (Json::Value::iterator itt2 = inVal.begin(); itt2 != inVal.end(); ++itt2) {
                        ...
                        if ((*itt2)["level"].empty() == false) {
                            ...
                            string control_id = to_string(_device.instanceID) + "_" +
                                                to_string(_device.commandClassID) + "_" +
                                                to_string(_device.sensorID);
                            InsertControl(_device, control_id);
                        }
                    }
                }
            ...
            if (instance["commandClasses"]["50"].empty() == false) {
                ...
                for (Json::Value::iterator itt2 = inVal.begin(); itt2 != inVal.end(); ++itt2) {
                    ...
                    if (((_device.scaleID == 0) || (_device.scaleID == 2) ||
                         (_device.scaleID == 4) || (_device.scaleID == 5) ||
                         (_device.scaleID == 6)) &&
                        (sensorType == 1)) {
                        ...
                        if (scaleString == "kWh") {
                            ...
                            cerr << "Razberry: Device Scale not handled at the moment, please "
                                    "report (nodeID:"
                                 << _device.nodeID << " instanceID:" << _device.instanceID
            ...
            if (instance["commandClasses"]["50"].empty() == false) {
                ...
                for (Json::Value::iterator itt2 = inVal.begin(); itt2 != inVal.end(); ++itt2) {
                    ...
                    if (scaleString == "kWh") {
                        ...
                        cerr << "Razberry: Device Scale not handled at the moment, please report "
                                "(nodeID:"
                             << _device.nodeID << " instanceID:" << _device.instanceID
                             << " Scale:" << scaleString.c_str() << ")\n";
                        continue;
            ...
            if (instance["commandClasses"]["67"].empty() == false) {
                ...
                for (Json::Value::iterator itt2 = inVal.begin(); itt2 != inVal.end(); ++itt2) {
                    ...
                    _device.floatValue = (*itt2)["val"]["value"].asFloat();
                    string control_id = to_string(_device.instanceID) + "_" +
                                        to_string(_device.commandClassID) + "_" + "_" + sKey;
                ...
                for (Json::Value::iterator itt2 = inVal.begin(); itt2 != inVal.end(); ++itt2) {
                    ...
                    _device.intvalue =
                        (*itt2)["sensorState"]["value"].asInt(); // sensorSate or sensorState ??
                    string control_id = to_string(_device.instanceID) + "_" +
                                        to_string(_device.commandClassID) + "_" + sKey;
...
void CRazberry::UpdateDevice(const std::string& path, const Json::Value& obj)
{
    ...
    if ((path.find("srcId") != std::string::npos) || (path.find("sensorTime") != std::string::npos))
        return;
    ...
    if (path.find("instances.0.commandClasses.128.data.last") != std::string::npos) {
        ...
        // COMMAND_CLASS_SENSOR_MULTILEVEL
        // Possible fix for Everspring ST814. maybe others, my multi sensor is coming soon to find
        // out!
        std::vector<std::string> results;
        results = StringSplit(path, ".");
            ...
            if (cmdID == 43) {
                ...
                if (instanceID == 0) {
                    ...
                    if (pDevice == NULL) {
                        ...
                        _device.isFLiRS =
                            !_device.isListening && (_device.sensor250 || _device.sensor1000);
                        ...
                        InsertControl(_device, "");
                        pDevice =
                            FindDevice(devID, instanceID, indexID, cmdID, ZDTYPE_SWITCH_NORMAL);
                    }
                }
            }
        }
    ...
    if (pDevice == NULL) {
        ...
        for (int iInst = 0; iInst < 7; iInst++) {
            ...
            std::map<std::string, _tZWaveDevice>::iterator iDevice =
                m_devices.find(devicestring_id);
            if (iDevice != m_devices.end()) {
                bFoundInstance = true;
                break;
            }
        }
        if (!bFoundInstance) _device.instanceID = oldinstance;
        ...
        if (iDevice == m_devices.end()) return; // uhuh?
...
//	return; //to soon
    ...
    case ZDTYPE_SWITCH_DIMMER: {
        ...
        if (pDevice->intvalue == intValue) {
            ...
            if (pDevice->sequence_number == 0) pDevice->sequence_number = 1;
            return;
        }
    ...
    if (to_publish) Owner->PublishControl(pDevice->DeviceId, pDevice->ControlId, value);
    pDevice->lastreceived = atime;
    pDevice->sequence_number += 1;
    if (pDevice->sequence_number == 0) pDevice->sequence_number = 1;
}

ZWaveBase::_tZWaveDevice* CRazberry::FindDeviceByControl(const string& dev_id,
...
ZWaveBase::_tZWaveDevice* CRazberry::FindDeviceByNodeId(const int node_id)
{
    std::map<std::string, _tZWaveDevice>::iterator itt;
    for (itt = m_devices.begin(); itt != m_devices.end(); ++itt) {
        if (itt->second.nodeID == node_id) return &itt->second;
    }
    return NULL;
}
...
ZWaveBase::_tZWaveDevice* CRazberry::FindDeviceByScale(const int nodeID, const int scaleID)
{
    std::map<std::string, _tZWaveDevice>::iterator itt;
    for (itt = m_devices.begin(); itt != m_devices.end(); ++itt) {
        if ((itt->second.nodeID == nodeID) && (itt->second.scaleID == scaleID)) return &itt->second;
    }
    return NULL;
}
...
void CRazberry::SwitchOn(const _tZWaveDevice& device, const string& value)
{
    ...
    if ((device.commandClassID == 64) && (iValue != 0)) iValue = 1;
    sstr << "devices[" << device.nodeID << "].instances[" << device.instanceID
         << "].commandClasses[" << device.commandClassID << "].Set(" << iValue << ")";
    RunCMD(sstr.str());
}
...
void CRazberry::SetThermostatSetPoint(const _tZWaveDevice& device, const string& value)
{
    std::stringstream sstr;
    sstr << "devices[" << device.nodeID << "].instances[" << device.instanceID
         << "].commandClasses[" << device.commandClassID << "].Set(" << device.sensorID << ","
         << value << ",null)";
    RunCMD(sstr.str());
}
